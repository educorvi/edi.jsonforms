...
class OptionList(Item):
    ...
    def get_options(self) -> list[str]:  # noqa: C901
        ...
        expects the response to be a JSON array of objects, e.g.
                    [{"id": "1", "name": "Option 1"}, {"id": "2", "name": "Option 2"}]
            - if id_mapping and value_mapping are set and exist in the item, use them
              for id and value, e.g. "1:Option 1"
            - if id_mapping and value_mapping are not set or do not exist in the item,
              use all key-value pairs, e.g. "id 1 name Option 1"
        """
